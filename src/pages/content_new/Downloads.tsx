...
export default function Downloads() {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (editingId) {
        const { error } = await supabase.from('downloads').update(cleanValues).eq('id', editingId);
        if (error) throw error;
        toast.success('Downloads updated successfully');
      } else {
        const { error } = await supabase.from('downloads').insert([cleanValues]);
        if (error) throw error;
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingId ? 'Edit' : 'Add'} Downloads</DialogTitle>
            </DialogHeader>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <FormField control={form.control} name="file_url" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <FileUpload value={field.value || ''} onChange={field.onChange} bucket="documents" folder="file_urls" accept="*/*" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <FormField control={form.control} name="file_type" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <FileUpload value={field.value || ''} onChange={field.onChange} bucket="documents" folder="file_types" accept="*/*" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <FormField control={form.control} name="file_size" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <FileUpload value={field.value || ''} onChange={field.onChange} bucket="documents" folder="file_sizes" accept="*/*" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
