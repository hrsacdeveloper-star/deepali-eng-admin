...
export default function Careers() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('careers').select('*').order('created_at', { ascending: false }).limit(100);
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (editingId) {
        const { error } = await supabase.from('careers').update(cleanValues).eq('id', editingId);
        if (error) throw error;
        toast.success('Careers updated successfully');
      } else {
        const { error } = await supabase.from('careers').insert([cleanValues]);
        if (error) throw error;
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingId ? 'Edit' : 'Add'} Careers</DialogTitle>
            </DialogHeader>
            <Form {...form}>
            ...
            {loading ? (
              ...
              dataList.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="max-w-xs truncate">{item.department}</TableCell>
