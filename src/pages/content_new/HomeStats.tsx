...
export default function HomeStats() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('home_stats').select('*').order('created_at', { ascending: false }).limit(100);
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (editingId) {
        const { error } = await supabase.from('home_stats').update(cleanValues).eq('id', editingId);
        if (error) throw error;
        toast.success('Home Stats updated successfully');
      } else {
        const { error } = await supabase.from('home_stats').insert([cleanValues]);
        if (error) throw error;
        toast.success('Home Stats created successfully');
      }
      setOpen(false);
      fetchData();
    } catch (error: any) {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Home Stats Management</h1>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingId ? 'Edit' : 'Add'} Home Stats</DialogTitle>
            </DialogHeader>
            <Form {...form}>
            ...
            {loading ? (
              ...
              dataList.map((item) => (
                <TableRow key={item.id}>
                  <TableCell className="max-w-xs truncate">{item.label}</TableCell>
                  <TableCell className="max-w-xs truncate">{item.value}</TableCell>
                  <TableCell className="max-w-xs truncate">{item.suffix}</TableCell>
                  <TableCell className="max-w-xs truncate">{item.icon}</TableCell>
