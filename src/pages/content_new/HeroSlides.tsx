...
export default function HeroSlides() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('hero_slides').select('*').order('created_at', { ascending: false }).limit(100);
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (editingId) {
        const { error } = await supabase.from('hero_slides').update(cleanValues).eq('id', editingId);
        if (error) throw error;
        toast.success('Hero Slides updated successfully');
      } else {
        const { error } = await supabase.from('hero_slides').insert([cleanValues]);
        if (error) throw error;
        toast.success('Hero Slides created successfully');
      }
      setOpen(false);
      fetchData();
    } catch (error: any) {
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingId ? 'Edit' : 'Add'} Hero Slides</DialogTitle>
            </DialogHeader>
            <Form {...form}>
            ...
            {loading ? (
              ...
              dataList.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="max-w-xs truncate">{item.button_text}</TableCell>
