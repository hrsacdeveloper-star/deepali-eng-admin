...
export default function CoreValues() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('core_values').select('*').order('created_at', { ascending: false }).limit(100);
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (editingId) {
        const { error } = await supabase.from('core_values').update(cleanValues).eq('id', editingId);
        if (error) throw error;
        toast.success('Core Values updated successfully');
      } else {
        const { error } = await supabase.from('core_values').insert([cleanValues]);
        if (error) throw error;
        toast.success('Core Values created successfully');
      }
      setOpen(false);
      fetchData();
    } catch (error: any) {
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingId ? 'Edit' : 'Add'} Core Values</DialogTitle>
            </DialogHeader>
            <Form {...form}>
