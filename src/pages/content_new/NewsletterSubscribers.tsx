...
export default function NewsletterSubscribers() {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (editingId) {
        const { error } = await supabase.from('newsletter_subscribers').update(cleanValues).eq('id', editingId);
        if (error) throw error;
        toast.success('Newsletter Subscribers updated successfully');
      } else {
        const { error } = await supabase.from('newsletter_subscribers').insert([cleanValues]);
        if (error) throw error;
        toast.success('Newsletter Subscribers created successfully');
      }
      setOpen(false);
      fetchData();
    } catch (error: any) {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Newsletter Subscribers Management</h1>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingId ? 'Edit' : 'Add'} Newsletter Subscribers</DialogTitle>
            </DialogHeader>
            <Form {...form}>
