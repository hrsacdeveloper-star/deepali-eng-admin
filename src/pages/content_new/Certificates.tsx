...
export default function Certificates() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('certificates').select('*').order('created_at', { ascending: false }).limit(100);
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (editingId) {
        const { error } = await supabase.from('certificates').update(cleanValues).eq('id', editingId);
        if (error) throw error;
        toast.success('Certificates updated successfully');
      } else {
        const { error } = await supabase.from('certificates').insert([cleanValues]);
        if (error) throw error;
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingId ? 'Edit' : 'Add'} Certificates</DialogTitle>
            </DialogHeader>
            <Form {...form}>
