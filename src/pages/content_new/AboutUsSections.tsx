...
const formSchema = z.object({
  ...
  secondary_image_url: z.string().optional().nullable(),
...
export default function AboutUsSections() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('about_us_sections').select('*').order('created_at', { ascending: false }).limit(100);
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (editingId) {
        const { error } = await supabase.from('about_us_sections').update(cleanValues).eq('id', editingId);
        if (error) throw error;
        toast.success('About Us Sections updated successfully');
      } else {
        const { error } = await supabase.from('about_us_sections').insert([cleanValues]);
        if (error) throw error;
        toast.success('About Us Sections created successfully');
      }
      setOpen(false);
      fetchData();
    } catch (error: any) {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">About Us Sections Management</h1>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingId ? 'Edit' : 'Add'} About Us Sections</DialogTitle>
            </DialogHeader>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <FormField control={form.control} name="secondary_image_url" render={({ field }) => (
                  <FormItem>
                    <FormLabel>Secondary Image Url</FormLabel>
                    <FormControl>
                      <FileUpload value={field.value || ''} onChange={field.onChange} bucket="images" folder="secondary_image_urls" accept="image/*" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
