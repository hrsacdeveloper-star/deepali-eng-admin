...
export default function Clients() {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (editingId) {
        const { error } = await supabase.from('clients').update(cleanValues).eq('id', editingId);
        if (error) throw error;
        toast.success('Clients updated successfully');
      } else {
        const { error } = await supabase.from('clients').insert([cleanValues]);
        if (error) throw error;
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingId ? 'Edit' : 'Add'} Clients</DialogTitle>
            </DialogHeader>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <FormField control={form.control} name="logo_url" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <FileUpload value={field.value || ''} onChange={field.onChange} bucket="images" folder="logo_urls" accept="image/*" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
            ...
            {loading ? (
              ...
              dataList.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>{item.logo_url ? <img src={item.logo_url} alt="" className="w-12 h-12 object-cover rounded" /> : 'None'}</TableCell>
                  <TableCell className="max-w-xs truncate">{item.order_index}</TableCell>
