...
export default function ToolRoomMachines() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('tool_room_machines').select('*').order('created_at', { ascending: false }).limit(100);
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (editingId) {
        const { error } = await supabase.from('tool_room_machines').update(cleanValues).eq('id', editingId);
        if (error) throw error;
        toast.success('Tool Room Machines updated successfully');
      } else {
        const { error } = await supabase.from('tool_room_machines').insert([cleanValues]);
        if (error) throw error;
        toast.success('Tool Room Machines created successfully');
      }
      setOpen(false);
      fetchData();
    } catch (error: any) {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Tool Room Machines Management</h1>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingId ? 'Edit' : 'Add'} Tool Room Machines</DialogTitle>
            </DialogHeader>
            <Form {...form}>
            ...
            {loading ? (
              ...
              dataList.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="max-w-xs truncate">{typeof item.specifications === 'string' ? item.specifications : JSON.stringify(item.specifications)}</TableCell>
