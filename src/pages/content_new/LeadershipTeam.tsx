...
export default function LeadershipTeam() {
  ...
  const fetchTeam = async () => {
    setLoading(true);
    const { data, error } = await supabase.from('leadership_team').select('*').order('order_index', { ascending: true });
  ...
  const onSubmit = async (values: TeamFormValues) => {
    if (editingId) {
      const { error } = await supabase.from('leadership_team').update(values).eq('id', editingId);
      if (error) toast.error('Failed to update member');
      else { toast.success('Member updated successfully'); setOpen(false); fetchTeam(); }
    } else {
      const { error } = await supabase.from('leadership_team').insert([values]);
    ...
    <div className="space-y-6 p-4 md:p-6 lg:p-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Manage your company's leadership team.</p>
        </div>
