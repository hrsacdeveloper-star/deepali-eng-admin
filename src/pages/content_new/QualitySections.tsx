...
export default function QualitySections() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('quality_sections').select('*').order('created_at', { ascending: false }).limit(100);
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (editingId) {
        const { error } = await supabase.from('quality_sections').update(cleanValues).eq('id', editingId);
        if (error) throw error;
        toast.success('Quality Sections updated successfully');
      } else {
        const { error } = await supabase.from('quality_sections').insert([cleanValues]);
        if (error) throw error;
        toast.success('Quality Sections created successfully');
      }
      setOpen(false);
      fetchData();
    } catch (error: any) {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Quality Sections Management</h1>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingId ? 'Edit' : 'Add'} Quality Sections</DialogTitle>
            </DialogHeader>
            <Form {...form}>
