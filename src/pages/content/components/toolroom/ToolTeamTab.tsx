...
export function ToolTeamTab() {
  ...
  const form = useForm<TeamFormValues>({
    ...
    defaultValues: { name: '', role: '', bio: '', image: '', display_order: 0 },
  });

  const fetchTeam = async () => {
    setLoading(true);
    const { data, error } = await supabase.from('tool_room_team').select('*').order('display_order', { ascending: true });
  ...
  const onSubmit = async (data: TeamFormValues) => {
    try {
      if (editingId) {
        const { error } = await supabase.from('tool_room_team').update(data).eq('id', editingId);
        if (error) throw error;
        toast.success('Team member updated');
      } else {
        const { error } = await supabase.from('tool_room_team').insert([data]);
        if (error) throw error;
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { error } = await supabase.from('tool_room_team').delete().eq('id', id);
      if (error) throw error;
  ...
  const openCreate = () => {
    setEditingId(null);
    form.reset({ name: '', role: '', bio: '', image: '', display_order: 0 });
    setOpen(true);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-foreground">Tool Room Team</h3>
        ...
        <Table className="[&>div]:max-w-full min-w-max">
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow><TableCell colSpan={5} className="text-center py-6 text-muted-foreground">No members found</TableCell></TableRow>
            ) : (
              team.map(member => (
