...
export function ToolFacilitiesTab() {
  ...
  const form = useForm<FacFormValues>({
    ...
    defaultValues: { name: '', description: '', image: '', display_order: 0 },
  });

  const fetchFacilities = async () => {
    setLoading(true);
    const { data, error } = await supabase.from('tool_room_facilities').select('*').order('display_order', { ascending: true });
  ...
  const onSubmit = async (data: FacFormValues) => {
    try {
      if (editingId) {
        const { error } = await supabase.from('tool_room_facilities').update(data).eq('id', editingId);
        if (error) throw error;
        toast.success('Facility updated');
      } else {
        const { error } = await supabase.from('tool_room_facilities').insert([data]);
        if (error) throw error;
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { error } = await supabase.from('tool_room_facilities').delete().eq('id', id);
      if (error) throw error;
  ...
  const openCreate = () => {
    setEditingId(null);
    form.reset({ name: '', description: '', image: '', display_order: 0 });
    setOpen(true);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-foreground">Tool Room Facilities</h3>
        ...
        <Table className="[&>div]:max-w-full min-w-max">
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow><TableCell colSpan={5} className="text-center py-6 text-muted-foreground">No facilities found</TableCell></TableRow>
            ) : (
              facilities.map(fac => (
                <TableRow key={fac.id}>
